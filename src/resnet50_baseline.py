"""
ResNet50 baseline for binary arch recognition in architectural heritage images.

Final experimental configuration used in the study:
...
CLASS_TO_LABEL = {"No Arch": 0, "Has Arch": 1}
LABEL_TO_CLASS = {0: "No Arch", 1: "Has Arch"}
...
class SplitDataset(Dataset):
    """
    Dataset driven by the deposited fixed split CSV files.
    ...
    `dataset_root` should contain class folders compatible with
    relative_reference, e.g.:
        dataset_root/
            has_arch/
                image_001.jpg
            no_arch/
    ...
    def _resolve_path(self, row):
        ...
        # Compatibility with the original Google Drive folder layout:
        # has_arch/has_arch and no_arch/no_arch
...
def build_model(device):
    ...
    # Partial fine-tuning: final residual stage.
...
def main():
    ...
    parser.add_argument("--output-dir", default="outputs/resnet50_baseline")
    parser.add_argument(
        "--evaluate-only",
        action="store_true",
    ...
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Checkpoint path used with --evaluate-only.",
    )
    ...
    # Fixed split verification from the final study.
    ...
    # Final ResNet50 implementation values.
    ...
    checkpoint_path = output_dir / "best_resnet50_baseline.pth"
